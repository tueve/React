...
export class NPMDashBoard extends React.Component {
  ...
  componentDidMount() {
    ...
    if (search) {
      if (!hash) {
        ...
        this.props.setCurrentPackage(search.replace('?', ''));
      } else {
  ...
  checkPackageInCompareList = (packageItem, compareList) => {
    if (compareList.length === 0) return;
    const getPackage = compareList.find(item => item.name === packageItem);
  ...
  render() {
    const { compareList, currentPackageInfo } = this.props.package;
    const { loading, timeDuration, readme } = this.props;
    return (
      <div>
        <Helmet>
        ...
        <div className="row">
          ...
          {compareList.length > 1 && (
            ...
            >
              <div className="col-12">
                <div className="col-12">
                  <Button onClick={this.onCompareHandle}>COMPARE</Button>
                </div>
              </div>
            </Link>
          )}
          {!loading &&
            currentPackageInfo.length !== 0 && (
              <div className="col-12">
                <NPMChart
                  compareData={
                    !this.props.compareMode ? currentPackageInfo : compareList
                  }
                  filter={timeDuration}
          ...
          {currentPackageInfo.length > 0 &&
            !this.props.compareMode && (
              <NPMDetail
                {...dataConvert(currentPackageInfo)}
                onAddPackage={this.onAddHandle}
              />
            )}
          {/* <NPMCompare /> */}
          ...
          {!loading &&
            !this.props.compareMode && (
              <ReadMe className="row col-12 justify-content-center readme-content">
                <div className="col-12">
                  {ReactHtmlParser(this.props.readme)}
                </div>
              </ReadMe>
            )}
        </div>
...
const mapDispatchToProps = dispatch => ({
  ...
  setCurrentPackage: packageName => dispatch(setCurrentPackage(packageName)),
});
...
export default compose(withReducer, withSaga, withConnect)(NPMDashBoard);
